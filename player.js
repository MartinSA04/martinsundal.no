...
function add_polar_vectors(magnitude1, angle1, magnitude2, angle2) {
...
function set_context_font(style) {
...
function collisionDetection(object1, object2) {
    ...
    let overlap_distance = Math.sqrt(Math.pow(crossWidth, 2) + Math.pow(crossHeight, 2));
    let rebound_factor = 1000;
    let angle = Math.atan2(dy, dx);
    let acceleration_vector2 = add_polar_vectors(object2.acceleration, object2.acceleration_angle, overlap_distance * rebound_factor, angle);
    object2.acceleration_angle = acceleration_vector2[1] + Math.PI;
    object2.acceleration = acceleration_vector2[0];
    let acceleration_vector = add_polar_vectors(object1.acceleration, object1.acceleration_angle, overlap_distance * rebound_factor, angle);
    object1.acceleration_angle = acceleration_vector[1];
    object1.acceleration = acceleration_vector[0];
    return true;
}
class GameObject {
    constructor(x, y, width, height, friction = 0.95) {
        this.speed = [0, 0];
...
class GamePlayer extends GameObject {
    ...
    get_sprite() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let sprite = yield window.createImageBitmap(this.sprite_sheet, sx, sy, 19, 26);
            return sprite;
        });
    }
    draw(dt) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let sprite = yield this.get_sprite();
    ...
    update(dt) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const speedMagnitude = Math.sqrt(Math.pow(this.speed[0], 2) + Math.pow(this.speed[1], 2));
...
function sort_words_to_lines(words, width) {
    let lines = [];
    let line = '';
    words.forEach((word) => {
...
function create_section(text, bounding_rect, style) {
    let section = [];
    let dx = 0;
    let dy = 0;
...
function add_text_elements_from_html_object(html_object) {
...
function iterate_html_collection(collection) {
