class SlidingBlockPart {
    ...
    constructor(x: number, y: number, x_pos: number, ypos: number, image: ImageBitmap, context: CanvasRenderingContext2D) {
        this.x = x;
        this.y = y;
        this.x_pos = x_pos;
        this.y_pos = ypos;
...
class SlidingBlockGame {
    ...
    private parts: SlidingBlockPart[] = [];
    private current_empty: number[] = [0, 0];
    ...
    constructor(image: HTMLImageElement, context: CanvasRenderingContext2D) {
        this.image = image;
        this.context = context;
    }
    ...
    async slice_image(rows: number = 5, cols: number = 5) {
        ...
        for (let i = 0; i < rows; i++) {
            for (let j = 0; j < cols; j++) {
                ...
                const part = await window.createImageBitmap(
                    this.image, 
                    i * this.sub_width, 
                    j * this.sub_height, 
                    this.sub_width, 
    ...
    get_relevant_parts(x: number, y: number) {
        var parts: SlidingBlockPart[] = [];
    ...
    async animate_move(parts: SlidingBlockPart[], x: number, y: number) {
    ...
    async move(x: number, y: number) {
        ...
        await this.animate_move(parts, x, y);
