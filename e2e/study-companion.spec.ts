...
test("every rendered widget reveals once its source is in view", async ({ page }) => {
  await page.goto("/projects/study-companion/");
  const widgets = page.locator("[data-sc-out]");
  await expect(widgets).toHaveCount(3);
  for (let i = 0; i < 3; i++) {
    await expect(widgets.nth(i)).toHaveAttribute("data-shown", "", { timeout: 8000 });
    await expect(widgets.nth(i)).toHaveCSS("opacity", "1");
  }
});

test("widgets are all visible with JavaScript disabled", async ({ browser }) => {
  ...
  expect((await page.locator(".sc-split").innerText()).length).toBeGreaterThan(100);
  await ctx.close();
});
