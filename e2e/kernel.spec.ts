...
test("a world override repaints the kit without touching components", async ({ page }) => {
  ...
  const before = await rule.evaluate((el) => getComputedStyle(el).borderTopColor);
  await page.evaluate(() => {
    document.documentElement.style.setProperty("--hair", "rgb(255, 0, 0)");
  });
  const after = await rule.evaluate((el) => getComputedStyle(el).borderTopColor);
...
test("Barcode is deterministic for a given seed", async ({ page }) => {
  ...
  const first = await page.locator(alpha).evaluateAll((els) => els.map((e) => e.innerHTML));
  expect(first.length).toBeGreaterThan(1);
  expect(new Set(first).size).toBe(1);

  // And stable across a reload.
  await page.reload();
  const second = await page.locator(alpha).evaluateAll((els) => els.map((e) => e.innerHTML));
  expect(second).toEqual(first);

  // A different seed must produce a different strip, or the hash is useless.
  const bravo = await page.locator("[data-micro='barcode'][data-seed='bravo']").first().innerHTML();
